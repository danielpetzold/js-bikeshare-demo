...
# redeploys webapp. If "jasperserver" database does not exist, it 
# redeploys minimal database
# Additional "init" only calls init_database, which will try to recreate 
# database and fail if DB exists.

# Sets script to fail if any of commands fail 
set -e

# set the default_master.properties for buildomatic

initialize_jasperserver_properties() {

  # if environment not set, will use default values of postgres
...
_EOL_
  ...
  if [[ -f "$JRS_CUSTOMIZATION/default_master_additional.properties" ]]; then
    cat $JRS_CUSTOMIZATION/default_master_additional.properties >> /usr/src/jasperreports-server/buildomatic/default_master.properties
  fi
}
...
config_ssl() {
  ...
  if "$JRS_HTTPS_ONLY" = "true" ; then
    ...
    echo "NOT! Setting HTTPS only within JasperReports Server. Should actually turn it off."
  fi
...
config_customization() {
  ...
  # any properties files found in JRS_CUSTOMIZATION will be appended to
  # those that exist in the WAR
  #JRS_PROPERTIES_FILES=`find $JRS_CUSTOMIZATION -iname "*.properties" \
  #  -exec readlink -f {} \; | sort -V`
  #for propertyFile in $JRS_PROPERTIES_FILES; do
  #  if [[ -f "$propertyFile" ]]; then
	  # baseFilePath=`echo "$propertyFile" | grep -o "[0-9.]\+"`
#	  GREPEXPR="${JRS_CUSTOMIZATION//\//\\\/}(.+*)"
#	  echo "grep expression: <$GREPEXPR>"
#	  if [[ $propertyFile =~ $GREPEXPR ]]; then
#		baseFilePath=${BASH_REMATCH[1]}
#	  else
#	    baseFilePath=$propertyFile
#	  fi
#     echo "Appending $propertyFile to $baseFilePath"
#     cat "$propertyFile" >> $CATALINA_HOME/webapps/jasperserver-pro/$baseFilePath
#    fi
#  done
...
echo "$HOST_IP  $DB_HOST" >> /etc/hosts
